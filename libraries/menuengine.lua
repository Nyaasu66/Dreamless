...
-- Constructor
function menuengine.new(x, y, font, space, align, limit, addto_rootview)
    local self = {}
    ...
    addto_rootview = addto_rootview or true
    ...
    -- Separate from RootView
    function self:separate()
    ...
    -- Disable this Menu; "update" will have no effects.
...
function menuengine.draw()
    local i
    for i=1,#menus do
        if menus[#menus + 1 - i] then
            menus[#menus + 1 - i]:draw()
        end
    end
end
